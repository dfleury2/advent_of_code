...
private:
    ...
    long long get_value(std::vector<long long>& op_codes, int index, parameter_mode mode) const
    {
        switch (mode) {
...
public:
    ...
    virtual void get_output(long long output) = 0;
    ...
    void compute()
    {
        for (long long i = 0; i < (long long)op_codes.size();) {
            ...
            case 4: { // cout
                long long output = get_value(op_codes, i + 1, mode_1);
                get_output(output);
                i += 2;
                break;
            }
